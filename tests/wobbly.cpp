...
namespace
{
        ...
        public ::testing::Test
    {
        public:
            ...
            const wobbly::Point Position = wobbly::Point(100, 50);
    ...
    wobbly::Vector
    ForceForSpring(const wobbly::Object &first,
                   const wobbly::Object &second,
                   const wobbly::Vector &desired)
    {
    ...
    {
        const double justBelowThreshold = FirstPositionX -
    ...
    constexpr unsigned int GridWidth = 4;
    constexpr unsigned int GridHeight = 4;
        ...
        public Test
    {
        ...
        protected:
            ...
            typedef std::function<double (const wobbly::Point &)> ResultFactory;

            virtual void SetUp ();
            void InitializeWithDimensions (float width, float height);
            void ApplyTransformation (const MeshTransform &);

            std::function<double(int)>
            UnitDeformationFunction(const ResultFactory      &resultFactory,
                                    const wobbly::BezierMesh &mesh,
    ...
    {
        const unsigned int nHorizontalTiles = 4;
        const unsigned int nVerticalTiles = 4;

        float tileWidth = TileWidth (width, nHorizontalTiles);
        float tileHeight = TileHeight (height, nVerticalTiles);

        for (unsigned int i = 0; i < nVerticalTiles; ++i)
        {
            for (unsigned int j = 0; j < nHorizontalTiles; ++j)
            {
    ...
    void
    BezierMesh::ApplyTransformation(const MeshTransform &transform)
    {
    ...
    std::function<double (int)>
    BezierMesh::UnitDeformationFunction(const ResultFactory      &resultFactory,
                                        const wobbly::BezierMesh &mesh,
                                        unsigned int             nSamples)
    {
        auto deformation =
            [nSamples, &mesh, &resultFactory](int lookupValue) -> double {
                /* Divide by the total number of samples (2^10) in order to get
                 * the unit value. */

                const float unitLookupValue =
                    lookupValue / static_cast<float> (pow (2, nSamples));
                const auto lookup (wobbly::Point (unitLookupValue, 0.0));
                auto p (mesh.DeformUnitCoordsToMeshSpace (lookup));

                return resultFactory (p);
            };
    ...
    TEST_F (BezierMesh, LinearHorizontalDeformationForUniformScale)
    {
        ...
        std::function <double(int)> horizontalDeformation =
            UnitDeformationFunction ([](const wobbly::Point &p) -> double {
                                         return bg::get<0>(p);
                                     },
            mesh,
            nSamples);
